...
public class ProductPricingService : IProductPricingService
{
    ...
    {
        return CalculateProductPrice(productThumbnail.Price, productThumbnail.OldPrice, productThumbnail.SpecialPrice,
            productThumbnail.SpecialPriceStart, productThumbnail.SpecialPriceEnd);
    }

    public CalculatedProductPrice CalculateProductPrice(Product product)
    {
        return CalculateProductPrice(product.Price, product.OldPrice, product.SpecialPrice, product.SpecialPriceStart,
        ...
        DateTime? specialPriceStart, DateTime? specialPriceEnd)
    {
        ...
        if (specialPrice.HasValue && specialPriceStart < DateTime.Now && DateTime.Now < specialPriceEnd)
        {
            ...
            if (!oldPrice.HasValue || oldPrice < price) oldPrice = price;
        }
