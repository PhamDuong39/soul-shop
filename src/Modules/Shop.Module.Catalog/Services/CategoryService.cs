...
public class CategoryService : ICategoryService
{
    ...
    {
        var result = await _cache.GetAsync(AllCacheKey,
            async () => { return await _categoryRepository.Query().Include(c => c.Media).ToListAsync(); });
        return result;
    }

    public async Task ClearCache()
    {
        await Task.Run(() => { _cache.Remove(AllCacheKey); });
    }
    ...
    public async Task<Result<StandardTableResult<CategoryResult>>> List(StandardTableParam param)
    {
        ...
        if (gridData?.List?.Count() > 0)
            gridData.List.ToList().ForEach(c => { c.Name = IncludeParentName(all.FirstOrDefault(x => x.Id == c.Id)); });
    ...
    public IList<CategoryResult> GetChildrens(int id, IList<CategoryResult> all)
    {
        ...
        foreach (var item in items) list.AddRange(GetChildrens(item.Id, all));
        ...
        bool includeInMenu = true)
    {
        var all = await GetAllByCache();
        var list = all.Where(
            c => c.IsPublished == isPublished && c.IncludeInMenu == includeInMenu && c.ParentId == null);

        if (parentId.HasValue) list = list.Where(c => c.Id == parentId);
        ...
        bool includeInMenu = true)
    {
        ...
        foreach (var item in twoCategories.Where(c => c.SubCategories.Count > 0)) list.AddRange(item.SubCategories);
        return list;
    }
}
