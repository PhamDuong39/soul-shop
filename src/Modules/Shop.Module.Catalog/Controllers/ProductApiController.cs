...
/// <summary>
/// Product management API controller, responsible for product addition, deletion, modification, query and other management operations.
/// </summary>
...
public class ProductApiController : ControllerBase
{
    ...
    /// <summary>
    /// Get the product list by page, support filtering by product name, SKU and other conditions.
    /// </summary>
    /// <param name="param">Object containing paging and filtering parameters. </param>
    /// <returns>Return the paginated product list. </returns>
    [HttpPost("grid")]
    public async Task<Result<StandardTableResult<ProductQueryResult>>> List(
    ...
    public async Task<Result<ProductGetResult>> Get(int id)
    {
        ...
        var stocks = await _stockRepository.Query().Include(c => c.Warehouse)
            .Where(c => productIds.Contains(c.ProductId)).ToListAsync();
        ...
        var opIds = product.OptionValues.OrderBy(x => x.DisplayOrder).ThenBy(x => x.Option.Name)
            .GroupBy(c => c.OptionId).Select(c => c.Key).OrderBy(c => c);
        foreach (var optionId in opIds)
        {
            ...
            var result = new ProductGetOptionResult()
            {
                ...
                Values = product.OptionValues.Where(c => c.OptionId == optionId).Select(x =>
                    new ProductGetOptionValueResult
                    {
                        Id = x.Id,
                        Display = x.Display,
    ...
    /// <summary>
    /// Add a new product, support setting various attributes and related information of the product.
    /// </summary>
    /// <param name="param">Parameter object containing product creation information. </param>
    ...
    public async Task<Result> Add([FromBody] ProductCreateParam param)
    {
        ...
        if (product.StockTrackingIsEnabled) InitStock(stocks, stockHistories, product, currentUser, param.Stocks);
        ...
        foreach (var variation in variations)
        {
            ...
            if (child.StockTrackingIsEnabled) InitStock(stocks, stockHistories, child, currentUser, variation.Stocks);
            ...
            foreach (var combination in coms)
            {
                ...
                if (product.OptionCombinations.Any(c =>
                        c.OptionId == combination.OptionId && c.Value == combination.Value))
                    continue;
    ...
    /// </summary>
    /// <param name="id">Product ID. </param>
    /// <param name="param">Parameter object containing product update information. </param>
    /// <returns>Return the operation result. </returns>
    ...
    public async Task<Result> Edit(int id, [FromBody] ProductCreateParam param)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (product.IsPublished) product.UnpublishedOn = DateTime.Now;
            }
        }
        ...
        if (isPriceChanged) product.AddPriceHistory(currentUser);
        ...
        if (product.StockTrackingIsEnabled)
        {
            ...
            var initStockParams = param.Stocks.Distinct()
                .Where(c => !productStocks.Select(x => x.WarehouseId).Contains(c.Id));
            var updateStockParams = param.Stocks.Distinct()
                .Where(c => productStocks.Select(x => x.WarehouseId).Contains(c.Id));
        ...
        //If it is a product combination (SKU) being edited, it does not participate in the editing options
        //If it is the main product being edited, it participates in the editing options
        if (product.ParentGroupedProductId == null && product.ParentProduct == null)
        {
            ...
            var deleteOvs = product.OptionValues.Where(x =>
                options.All(c => c.Values.All(v => v.Value != x.Value) && x.OptionId != c.Id));
            ...
            {
                Product child = null;
                if (variation.Id > 0) child = product.Childrens.FirstOrDefault(c => c.Id == variation.Id);
                ...
                else
                {
                    ...
                    if (child.StockTrackingIsEnabled)
                    {
                        ...
                        var initStockParams = variation.Stocks.Distinct()
                            .Where(c => !childStocks.Select(x => x.WarehouseId).Contains(c.Id));
                        var updateStockParams = variation.Stocks.Distinct()
                            .Where(c => childStocks.Select(x => x.WarehouseId).Contains(c.Id));
                    ...
                    if (childIsPriceChanged) child.AddPriceHistory(currentUser);
                }
            }
        ...
        {
            //SKU Editable individual visible fields
        ...
        var deletedAttrValues =
            product.AttributeValues.Where(attrValue => attributes.All(x => x.AttributeId != attrValue.AttributeId));
        ...
        using (var transaction = _productRepository.BeginTransaction())
        {
            ...
            if (addStocks.Count > 0) _stockRepository.AddRange(addStocks);
            await _stockRepository.SaveChangesAsync();
    ...
    public async Task<Result> Delete(int id)
    {
        ...
        if (product == null) return Result.Fail("The document does not exist");

        var anyChild = await _productRepository.Query().AnyAsync(c => c.ParentGroupedProductId == product.Id);
        if (anyChild) return Result.Fail("The current product has a product combination, deletion is not allowed");
        ...
        if (anyOrder) return Result.Fail("The current product has been ordered and cannot be deleted.");
    ...
    /// <summary>
    /// Cancel the release of the product with the specified ID, making it invisible in the foreground.
    /// </summary>
    /// <param name="id">Product ID. </param>
    /// <returns>Return the operation result. </returns>
    ...
    /// </summary>
    /// <param name="id">Product ID. </param>
    /// <param name="model">An object containing optional parameters when cloning a product. </param>
    /// <returns>Return the operation result, including the ID of the newly cloned product. </returns>
    ...
    public async Task<Result> Clone(int id, [FromBody] ProductCloneParam model)
    {
        ...
        {
            var stocks = await _stockRepository.Query().Include(c => c.Warehouse).Where(c => c.ProductId == product.Id)
                .ToListAsync();
    ...
    private void InitStock(List<Stock> addStocks, List<StockHistory> addStockHistories, Product product, User user,
        IEnumerable<ProductCreateStockParam> stockParams)
    {
    ...
    private void UpdateAndDeleteStock(IEnumerable<Stock> productStocks, List<StockHistory> addStockHistories,
        Product product, User user, IEnumerable<ProductCreateStockParam> stockParams)
    {
        if (stockParams == null || stockParams.Count() <= 0)
            return;
