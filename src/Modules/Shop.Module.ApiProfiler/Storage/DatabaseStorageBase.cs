...
namespace Shop.Module.ApiProfiler.Storage;
...
public abstract class DatabaseStorageBase : IAsyncStorage, IDatabaseStorageConnectable
{
    ...
    protected DatabaseStorageBase(string connectionString, string profilersTable, string timingsTable,
        string clientTimingsTable)
    {
        ConnectionString = connectionString;
    ...
    public abstract IEnumerable<Guid> List(int maxResults, DateTime? start = null, DateTime? finish = null,
    ...
    /// <param name="orderBy">Result order.</param>
    /// <returns>The list of GUID keys.</returns>
    public abstract Task<IEnumerable<Guid>> ListAsync(int maxResults, DateTime? start = null, DateTime? finish = null,
        ListResultsOrder orderBy = ListResultsOrder.Descending);
    ...
    protected void ConnectTimings(MiniProfiler profiler, List<Timing> timings, List<ClientTiming> clientTimings)
    {
        if (profiler?.RootTimingId.HasValue == true && timings.Count > 0)
        {
            ...
            if (rootTiming != null)
            {
                ...
                foreach (var timing in timings) timing.Profiler = profiler;
    ...
    /// </summary>
    public List<string> TableCreationScripts =>
        _tableCreationScripts ?? (_tableCreationScripts = GetTableCreationScripts().ToList());
    ...
    DbConnection IDatabaseStorageConnectable.GetConnection()
    {
