...
public class Timing : IDisposable
{
    ...
    /// </summary>
    /// <param name="profiler">The <see cref="Soul.Shop.Module.ApiProfiler.MiniProfiler"/> this <see cref="Timing"/> belongs to.</param>
    ...
    public List<Timing> Children
    {
        ...
        set
        {
            if (value?.Count > 0)
                lock (value)
                {
                    foreach (var t in value) t.ParentTiming = this;
                }
    ...
    public decimal DurationWithoutChildrenMilliseconds
    {
        get
        {
            ...
            if (Children != null)
                lock (_syncRoot)
                {
                    foreach (var child in Children) result -= child.DurationMilliseconds.GetValueOrDefault();
                }
    ...
    public void Stop()
    {
        ...
        if (_minSaveMs.HasValue && _minSaveMs.Value > 0 && ParentTiming != null)
        {
            ...
            if (compareMs < _minSaveMs.Value) ParentTiming.RemoveChild(this);
        }
    }
    ...
    /// <remarks>
    /// Used outside this assembly for custom deserialization when creating an <see cref="IAsyncStorage"/> implementation.
    /// </remarks>
