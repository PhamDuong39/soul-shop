...
public static class Render
{
        ...
        bool? startHidden = null)
    {
        ...
        if (isAuthorized) sb.Append(" data-authorized=\"true\"");
        if (showTrivial ?? options.PopupShowTrivial) sb.Append(" data-trivial=\"true\"");
        if (showTimeWithChildren ?? options.PopupShowTimeWithChildren) sb.Append(" data-children=\"true\"");
        if (showControls ?? options.ShowControls) sb.Append(" data-controls=\"true\"");
        if (startHidden ?? options.PopupStartHidden) sb.Append(" data-start-hidden=\"true\"");
    ...
    public static string SingleResultHtml(MiniProfiler profiler, string path)
    {
        ...
        sb.Append(profiler.ToJson(true));
        sb.Append(";</script>");
        sb.Append(Includes(profiler, path, true));
    ...
    /// <param name="path">The root path that MiniProfiler is being served from.</param>
    /// <returns>A full HTML page for this MiniProfiler.</returns>
    public static string ResultListHtml(MiniProfilerBaseOptions options, string path, string queryString = null)
    {
  ...
  <head>
    ...
    <script>MiniProfiler.listInit({{path: '{path}', version: '{version}',queryString: '{queryString}'}});</script>
  </head>
  <body>
