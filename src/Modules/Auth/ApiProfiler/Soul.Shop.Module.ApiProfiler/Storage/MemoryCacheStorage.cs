...
public class MemoryCacheStorage : IAsyncStorage
{
    ...
    {
        return CacheKeyPrefix + id.ToString();
    }
        ...
        ListResultsOrder orderBy = ListResultsOrder.Descending)
    {
        ...
        {
            var idxStart = 0;
            var idxFinish = _profiles.Count - 1;
            ...
            if (orderBy == ListResultsOrder.Ascending)
                for (var i = idxStart; i <= idxFinish; i++)
                {
                    guids.Add(keys[i].Id);
                    if (guids.Count == maxResults) break;
                }
            else
                for (var i = idxFinish; i >= idxStart; i--)
                {
                    guids.Add(keys[i].Id);
                    if (guids.Count == maxResults) break;
                }
    ...
    {
        return _cache.Get(GetCacheKey(id)) as MiniProfiler;
    }
    ...
    public void Save(MiniProfiler profiler)
    {
        ...
        lock (_profiles)
        {
            if (!_profiles.ContainsKey(profileInfo)) _profiles.Add(profileInfo, null);
        ...
        if (!profiler.HasUserViewed) SetUnviewed(profiler.User, profiler.Id);
    }
