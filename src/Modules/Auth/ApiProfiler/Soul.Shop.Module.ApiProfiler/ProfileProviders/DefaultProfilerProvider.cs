...
namespace Soul.Shop.Module.ApiProfiler.ProfileProviders;
...
public class DefaultProfilerProvider : IAsyncProfilerProvider
{
    ...
    {
        return CurrentProfiler = new MiniProfiler(profilerName ?? nameof(MiniProfiler), options);
    }
    ...
    public virtual void Stopped(MiniProfiler profiler, bool discardResults)
    {
        ...
        if (discardResults)
        {
            if (CurrentProfiler == profiler) CurrentProfiler = null;
            return;
        }
    ...
    /// <summary>
    /// Calls <see cref="IAsyncStorage.Save"/> to save the current
    ...
    /// <summary>
    /// Asynchronously calls <see cref="IAsyncStorage.SaveAsync"/> to save the current
    /// profiler using the current storage settings. 
    /// If <see cref="MiniProfiler.Storage"/> is set, this will be used.
    /// </summary>
    /// <param name="profiler">The <see cref="MiniProfiler"/> to save.</param>
