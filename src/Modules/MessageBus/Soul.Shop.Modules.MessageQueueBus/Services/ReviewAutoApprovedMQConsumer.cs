    ...
    : IConsumer<ReviewAutoApprovedEvent>
{
    ...
    public async Task Consume(ConsumeContext<ReviewAutoApprovedEvent> context)
    {
        try
        {
            if (context?.Message?.ReviewId > 0) await mediator.Publish(context.Message);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Comments are automatically reviewed and processed, but processing fails",
                context?.Message);
        }
    }
}