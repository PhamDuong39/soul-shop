...
public class UploadApiController : ControllerBase
{
    ...
    {
        if (file == null) return Result.Fail("Please select upload file");
    ...
    {
        if (formCollection == null || formCollection.Files.Count <= 0) return Result.Fail("Please select upload files");
        ...
        foreach (var file in formCollection.Files)
            try
            {
                ...
                var originalFileName = headerValue.FileName.Trim('"');
                var fileName = $"{Guid.NewGuid().ToString("N")}{Path.GetExtension(originalFileName)}";
                var media = await _mediaService.SaveMediaAsync(file.OpenReadStream(), fileName, file.ContentType,
                    file.Length);
                list.Add(media);
            }
            catch (Exception ex)
