...
public partial class MemoryCacheManager : ILocker, IStaticCacheManager
{
    ...
    {
        foreach (var key in _allKeys.Where(p => !p.Value).Select(p => p.Key).ToList()) RemoveKey(key);
    }
    ...
    public async Task<T> GetAsync<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
    {
        ...
        if ((cacheTime ?? _config.CacheTimeInMinutes) > 0)
            Set(key, result, cacheTime ?? _config.CacheTimeInMinutes);
    ...
    {
        if (data != null) _cache.Set(AddKey(key), data, GetMemoryCacheEntryOptions(TimeSpan.FromMinutes(cacheTime)));
    }
    ...
    public virtual void RemoveByPattern(string pattern)
    {
        ...
        foreach (var key in matchesKeys) _cache.Remove(RemoveKey(key));
    }
