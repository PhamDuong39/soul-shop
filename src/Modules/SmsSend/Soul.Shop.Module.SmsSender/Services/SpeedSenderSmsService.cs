...
using Soul.Shop.Module.SmsSender.Models;

namespace Soul.Shop.Module.SmsSender.Services;

public class SpeedSenderSmsService(
    ILoggerFactory loggerFactory,
    IOptionsMonitor<SpeedSenderSmsOptions> options,
    ...
    : ISmsSender
{
    ...
    private readonly ILogger _logger = loggerFactory.CreateLogger<SpeedSenderSmsService>();
    ...
    public async Task<bool> SendSmsAsync(SmsSend model)
    {
        try
        {
            ...
            {
                var message = JsonConvert.DeserializeObject<SpeedSenderSmsResult>(result.Response);
                if (message?.Code == "OK")
                {
                    model.IsSucceed = true;
